...
public class OpenDoorWithKey : MonoBehaviour {
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.E) && !doorGo)
        {
            ...
            if (Vector3.Distance(playerTrans.position, this.transform.position) < 5f && Player.hasKey)
            {
                if (doorStatus)
                { //close door
                    StartCoroutine(this.MoveDoor2(doorClose));
                }
                else
                { //open door
                    StartCoroutine(this.MoveDoor2(doorOpen));
                }
            }
        }
    }

    public IEnumerator MoveDoor2(Quaternion dest)
    {
        doorGo = true;
